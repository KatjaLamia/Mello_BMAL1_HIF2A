...
This analysis includes `r nrow(COAD_N)` normal colon samples and `r nrow(COAD_T)` primary tumor samples from the TCGA-COAD project, `r nrow(LUAD_N)` normal lung samples and `r nrow(LUAD_T)` primary tumor samples from the TCGA-LUAD project, `r nrow(BRCA_N)` normal breast samples and `r nrow(BRCA_T)` primary tumor samples from the TCGA-BRCA project, `r nrow(RCC_N)` normal kidney samples and `r nrow(RCC_T)` primary tumor samples from the TCGA-KIRC project, and `r nrow(KIRP_N)` normal kidney samples and `r nrow(KIRP_T)` primary tumor samples from the TCGA-KIRP project. All of the "normal" samples are biopsies from patients with matched tumor samples. Not all tumor samples have matched normal adjacent samples.
...
```{r Calculate statistics using 2-way ANOVA}
